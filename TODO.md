...
- [ ] **The `jetty.version` pin has no effect.** `<jetty.version>12.1.11</jetty.version>` is
...
- [ ] **CI never runs on the default branch.** `.github/workflows/ci.yml:5` triggers
...
- [ ] **Conneg equivalence is weaker than the clause it tests.** The spec says the response
...
- [ ] **Redaction covers headers only.** `Redaction.redactHeaders` strips six header names;
...
- [ ] **`link:<rel>` bind extractor is implemented but unreachable.** `Executor.bind:210`
...
- [ ] **Header `equals` inspects only the first value** while `matches` and `contains` inspect
...
- [ ] **An uncatalogued requirement IRI silently downgrades a verdict.**
...
- [ ] **`RefLwsServer` emits an inline `@context`, so the self-test loop cannot see context
...
- [ ] **Docs drift:** `manifests/README.md` says core has **12 tests** (there are **21**) and
...
- [ ] **Record the changed premise in `DECISIONS.md`** — D-0002 pinned the June WD as the
      baseline and CLAUDE.md rule 2 makes a moved premise a dated entry. (D-0037 has been
      added alongside this file; extend it as the P1 items land.)
